...
1. On the **Create an access policy - Permissions** blade, perform the following actions, and then select **Next**:
...
1. On the **Create an access policy - Principal** blade, type your service principal name **securefunc**_[yourname]_ into the search box. The system-assigned managed identity you created previously in this lab will have the same name as the Azure Function resource. Select the item so it is displayed as the Selected Item, then select **Next**.

1. On the **Create an access policy - Application (optional)** blade, leave the search box empty and select **Next**.

1. On the **Create an access policy - Review + Create** blade, select **Create**
