...
1. On the **Add access policy** blade, perform the following actions, and then select **Add**:
    ...
    | **Select principal** link | Click on **None selected**. Find and then select the service principal named **securefunc**_[yourname]_. The system-assigned managed identity you created previously in this lab will have the same name as the Azure Function resource. |
