...
1. On the **Create a storage account** blade, on the **Basics** tab, perform the following actions, and select **Review + create**:
   ...
   ![Create a storage account blade](./Media/l07_create_a_storage_account.png)
...
1.  On the **Create key vault** blade, on the **Basics** tab, perform the following actions, and select **Review + create**:
   ...
   ![Create key vault blade](./Media/l07_create_key_vault.png)
...
1. On the **Create Function App** blade, on the **Basics** tab, perform the following actions, and then select **Next: Hosting**:
    ...
    ![Create Function App blade](./Media/l07_create_function_app.png)
...
1. On the **Hosting** tab, perform the following actions, and then select **Review + create**:
    ...
    ![The Hosting tab on the Create Function App blade ](./Media/l07_create_function_app_hosting_tab.png)
...
1. On the **Create a secret** blade, perform the following actions, and then select **Create**:
    ...
    ![The Create a secret blade ](./Media/l07_create_a_secret.png)
...
1. On the **Secret Version** pane, perform the following actions:
    a. Select **Show secret value** to find the value of the secret.
    b. Record the value of the **Secret Identifier** text box because you'll use this later in the lab.
...
1. On the **Add access policy** blade, perform the following actions and then select **Add**:
    ...
    | **Key permissions** drop-down list | 0 selected.       <!--Conan, this is a bit confusing to me. do you mean the value "0" is selected or do they need to select? Or do you mean none should be selected. Can we clarify as in "Ensure that none of the key permissions are selected. or Ensure 0 is selected. or Select 0." Same query for similar entries. -->                |
...
1. Run the following command to exit the **httprepl** application: <!--Conan, we called this a tool earlier. Just checking if "application" here shouls be changed to "tool"-- >
...
<!--Conan, this appears to miss an object. Can we add what we are deploy-->
...
1. On the **Functions** pane, select the the existing **FileParser** function.
...
1. On the taskbar, select the **Microsoft Edge** icon, and then go to the Azure portal (<https://portal.azure.com>).
