...
1. From the **Configuration** pane, perform the following actions:
    ...
    a. Leave the **deployment slot setting** text box set to its default value.
    b. Select **OK** to close the pop-up window and return to the **Configuration** section.
    c. Select **Save** from the blade to save your settings.  
    d. In the **Save Changes** confirmation pop-up dialog box, select **Continue**.
