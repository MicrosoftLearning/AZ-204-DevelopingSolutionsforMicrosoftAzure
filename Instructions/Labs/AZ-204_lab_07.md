...
1. On the **Create a storage account** blade, on the **Basics** tab, perform the following actions, and select **Review + create**:
   ...
   | **Resource group** section        | Select **Create new**, enter **ConfidentialStack**[DeploymentId], and then select **OK**. |
...
1. In the **Upload blob** window, perform the following actions, and then select **Upload**:
    ...
    | **File Explorer** window                       | Browse to **Allfiles C:\AllFiles\AZ-204-DevelopingSolutionsforMicrosoftAzure-master\Allfiles\Labs\07\Starter\func**, select the **records.json** file, and then select **Open**. |
