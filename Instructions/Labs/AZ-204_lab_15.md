...
1. Open the **File Explorer** window, browse to **Allfiles (F):\\Allfiles\\Labs\\15\\Starter\\webapp-lab15**, and then open a text editor.
...
1. Make sure to replace `<web-app-name>` with the name of your .NET 7 web application project.
...
1. Enter the following command, and then select Enter to change the current directory to the **Allfiles (F):\\Allfiles\\Labs\\15\\Starter\\webapp-lab15** directory that contains the lab files:

    ```
    cd 'F:\Allfiles\Labs\15\Starter\webapp-lab15\'
    ```
...
1. Replace <your-registry-name> with the name of your Container Registry and replace <image-name> with the name of your project.
    
    > **Note**: Wait for the build task to complete before moving forward with this lab.

1. Close the **Windows Terminal** pane.
