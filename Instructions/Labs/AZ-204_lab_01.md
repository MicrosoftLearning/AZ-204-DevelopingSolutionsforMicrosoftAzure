...
1. On the **Create a storage account** blade, on the **Basics** tab, perform the following actions, and then select **Review + create**:
    ...
    | **Resource group** section      | Enter **ManagedPlatform-[deployId]**. |
...
1. On the **Marketplace** search results blade, select the **App Services** result.
...
1. In the **Microsoft Edge** browser window, enter the email address and password (Username/Email: <inject key="AzureAdUserEmail"></inject> 
    * Azure Password: <inject key="AzureAdUserPassword"></inject>) for your Microsoft account, and then select **Sign in**.
...
1. In the **Configuration** section, perform the following actions, select **Save**, and then select **Continue**:
    ...
    | **Value** text box | Enter the web app URL that you copied previously in this lab. **Note**: Make sure you include the protocol, such as **https://**, in the URL that you copy into the **Value** text box for this application setting and Remove the **Backslash** from URL. Ex: **https://imgapixxxx.azurewebsites.net**|
