...
Given the dynamic nature of Microsoft cloud tools, you might experience Azure UI changes that occur after the development of this training content. These changes might cause the lab instructions and lab steps to not align correctly.
...
1.  On the **All services** blade, select **Storage Accounts**, and then review your list of storage account instances.
...
1.  On the **ManagedPlatform** blade, select the **imgstor**_yourname_ storage account that you created earlier in this lab.
...
1.	In the **Upload blob** window, perform the following actions:
    ...
    | **Overwrite if files already exist** check box | Ensure that the check box is  is selected, and then select **Upload**.                                    |
...
1.  On the **Create Web App** blade, review the tabs from the blade, such as **Basics**, **Deployment**, **Monitoring**, **Tags**, and **Review + create**.
...
1.  On the **ManagedPlatform** blade, select the **imgapi**_[yourname]_ web app that you created earlier in this lab.

    > **Note**: If the **imgapi**_[yourname]_ web app is not listed, select the **Show hidden types** check box to display it.
...
1.  In the **Configuration** section, perform the following actions, select **Save**, and then select **Continue**.
    ...
    | **Value** text box | Paste the storage connection string that you copied to Notepad earlier in this lab. |
...
1.  In the **Properties** section, copy the value of the **URL** hyperlink, and then paste it to Notepad. You'll use this value later in the lab.
    ...
    > **Note**: At this point, the web server at this URL will return a placeholder webpage. You've not deployed any code to the Web App yet. You'll deploy code to the Web App later in this lab.
...
1.  On the **ManagedPlatform** blade, select the **imgweb**_yourname_ web app that you created earlier in this lab.

    > **Note**: If the **imgweb**_yourname_ web app is not listed, select the **Show hidden types** check box to display it.
...
1.  In the **Configuration** section, perform the following actions, select **Save**, and then select **Continue**:
    ...
    | **Value** text box | Enter the web app URL that you copied earlier in this lab. **Note**: Make sure you include the protocol, such as **https://**, in the URL that you copy into the **Value** text box for this application setting.|
...
1.	Return to the currently open Windows Terminal **Command Prompt** window <!--Conan, I am not sure I have cpme across "Windows terminal" till now. Can you please confirm this is correct?-->. Wait for the sign-in process to finish.
...
1.  In the Azure portal, select the **Cloud Shell** icon to open a new PowerShell session. <!--Conan, there are some minor variations between your labs and Marcin;s labs in how you have worded the clean up instructions. We should ideally be consistent across the labs. Can you please advise which version to go with?-->
...
1.  Enter the following command, and then press Enter to delete the **ManagedPlatform** resource group:
