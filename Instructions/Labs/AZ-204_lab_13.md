...
1. On the **Create Web App** blade, on the **Basics** tab, perform the following actions, and then select the **Monitoring** tab:
    ...
    | **Resource group** section | Select **Create new**, enter `wProject-Sam-RG` and select **OK** |
    | **Name** text box | Enter `webapp-sam_[yourname]_` |
    | **Publish** section | Select **Code** |
    | **Runtime stack** drop-down list | Select **.NET 7 (STS)** |
    | **Operating System** section | Select **Windows** |
    | **Region** drop-down list | Select the **East US** region |
    | **Windows Plan (East US)** section | Select **Create new**, enter the value `ManagedPlan` in the **Name** text box and select **OK** |
...
1. On the **General settings** tab, select **On** for **Basic Auth Publishing Credentials**, select **Save** and then select **Continue**.
...
### Exercise 6: Enable custom domain and SSL on the web app

#### Task 1: Create a custom domain and add a SSL for the web app

To add a custom domain and SSL on your web app [click here to watch the video](./media/Lab13_add_custom_domain.mp4)  

1. Open the Azure portal and navigate to your web app.

1. On the web app blade, select **Custom domains** under the **Settings** section.

1. On the **Custom domains** blade, select **+ Add custom domain**.

1. On the **Add custom domain** window, perform the following actions:

   | Setting | Action |
   | -- | -- |
   | **Domain provider** check box | Select **All other domain services** |
   | **TLS/SSL certificate** check box | Select **App Service Managed Certificate** |
   | **TLS/SSL type** check box | Select **SNI SSL** |
   | **Domain** text box | Enter *your custom domain name* |
   | **Hostname record type** drop-down list | Select **CNAME** |

1. Copy the values of **CNAME** and **TXT** DNS records, from the **Domain validation** section.

1. Go to your domain DNS provider and create a **CNAME** and **TXT** records with the values that you copied from the **Domain validation** section.

   > **Note**: Learn more about [validate domain ownership](https://learn.microsoft.com/en-us/azure/app-service/app-service-web-tutorial-custom-domain?tabs=root%2Cazurecli#dns-record-types).

1. Back to Azure portal, on the **Add custom domain** window, select **Validate**.

1. Once custom domain is Validated, select **Add** to add the custom domain to your web app.

   > **Note**: Wait a few minutes for the custom domain to be added and propagated.
...
Congratulations, you have now successfully created a custom domain and SSL on your web app.


#### Review

This exercise is about adding a custom domain and SSL on a web app in Azure.
