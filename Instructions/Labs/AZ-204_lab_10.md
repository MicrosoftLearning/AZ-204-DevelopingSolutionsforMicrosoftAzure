...
1.  On the **Create namespace** blade, on the **Basics** tab, perform the following actions, and select **Review + create**:
    ...
    | **Resource group** section | Select existing resource group **AsyncProcessor**_[Deploymentid]_|
    | **Namespace name** text box | Enter **sbnamespace**_[Deploymentid]_.|
...
1.  In the **Open Folder** window, browse to **Allfiles C:\AllFiles\AZ-204-DevelopingSolutionsforMicrosoftAzure-master\Allfiles\Labs\10\Solution\\MessagePublisher**, and then select **Select Folder**.

1.  In the **Visual Studio Code** window, Right click on the MicrosoftPublisher.csproj, and then select **Open in Integrated Terminal**.
...
1.  At the terminal prompt, run the following command to create a new .NET project named **MessagePublisher** in the current folder:
    ...
    ```
    dotnet new console --name MessagePublisher --output . --force
    ```
...
1.  Select **Service Bus Explorer (preview)**.
...
1.  In the **Open Folder** window, browse to **Allfiles C:\AllFiles\AZ-204-DevelopingSolutionsforMicrosoftAzure-master\Allfiles\Labs\10\\Solution\MessageReader**, and then select **Select Folder**.

1.  In the **Visual Studio Code** window, Right click on the MicrosoftReader.csproj, and then select **Open in Integrated Terminal**.

1.  At the terminal prompt, run the following command to create a new .NET project named **MessageReader** in the current folder:

    ```
    dotnet new console --name MessageReader --output . --force
    ```
