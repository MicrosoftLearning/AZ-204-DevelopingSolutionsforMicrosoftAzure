...
Given the dynamic nature of Microsoft cloud tools, you might experience Azure UI changes that occur after the development of this training content. These changes might cause the lab instructions and lab steps to align correctly.
...
1.  On the **code editor** <!--Marcin, I made code editor bold, but I am not sure if it is named tab since it is not capitalized. Can you please confirm?--> tab for the **Program.cs** file, delete all the code in the existing file.

1.  In the **Explorer** pane of the **Visual Studio Code** window, open the **Program.cs** file.

1.  On the code editor <!--Marcin, same as before. Should code editor be bold-->tab for the **Program.cs** file, delete all the code in the existing file.
...
1.  Review the **Program.cs** file, which should now include (where the `<storage-connection-string>` placeholder represents the connection string to the target Azure Service Bus namespace): <!-- Marcin, can we use a screenshot here if at all we are going to repeat the lines of code? or is it better to leave it as code to allow them to copy and paste?-->
...
1.  Review the **Main** method, which should now consist of the following code: <!--Marcin, same question as before-->
...
1.  On the code editor <!--Marcin, should this be bold--> tab for the **Program.cs** file, delete all the code in the existing file.
...
In this exercise, you created new messages in the queue by using the .NET library for Storage queues. <!--Marcin, please confirm that both "In this exercise statements are needed here. If so, can we change the second sentence to "You also created new messages in the queue...."-->
...
1.  From the Cloud Shell pane, run the following command to delete the **AsyncProcessor** resource group:
