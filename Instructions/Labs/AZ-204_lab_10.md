---
lab:
    az204Title: 'Lab 10: Asynchronously processing messages by using Azure Service Bus Queues'
    az020Title: 'Lab 10: Asynchronously processing messages by using Azure Service Bus Queues'
    az204Module: 'Module 10: Develop message-based solutions'
    az020Module: 'Module 10: Develop message-based solutions'
---

# Lab 10: Asynchronously processing messages by using Azure Service Bus Queues
...
1.  In the Azure portal, use the **Search resources, services, and docs** text box at the top of the page to search for **Service Bus** and, in the list of results, select **Service Bus**.

1.  On the **Service Bus** blade, select **+ Create**. 

1.  On the **Create namespace** blade, on the **Basics** tab, perform the following actions:
    
    1.  Leave the **Subscription** text box set to its default value.
    
    1.  In the **Resource group** section, select **Create new**, enter **AsyncProcessor**, and then select **OK**.
    
    1.  In the **Namespace name** text box, enter **sbnamespace***[yourname]*.

    1.  In the **Region** drop-down list, select any Azure region in which deploy an Azure Service Bus.

    1.  In the **Pricing tier** drop-down list, select **Basic**.
...
1.  On the **SAS Policy: RootManageSharedAccessKey** pane, next to the **Primary Connection String** entry, click the **Copy to clipboard** button, and record the copied value. You'll use it later in this lab.

    > **Note**: It doesn't matter which of the two available keys you choose. They are interchangeable.

1.  On the Service Bus namespace blade, in the **Entities** section, select **Queues** and select **+ Queue**.
...
1.  Leave the browser window open. You will use it again later in this lab.
...
1.  Review the **Program.cs** file, which should now include (where the `<storage-connection-string>` placeholder represents the connection string to the target Azure Service Bus namespace):
...
1.  In the **Main** method, add the following code to initialize *client* of type **ServiceBusClient** that will provide connectivity to the Service Bus namespace and **sender** that will be responsible for sending messages:
    ...
    > **Note**: The Service Bus client is safe to cache and use as a singleton for the lifetime of the application. This is considered one of the best practices when publishing and reading messages on regular basis.
...
1.  Verify that the console message displayed at the terminal prompt states that a batch of 3 messages has been published to queue.

1.  Select **Kill Terminal** (the **Recycle Bin** icon) to close the terminal pane and any associated processes.

1.  Switch to the Microsoft Edge browser displaying the Service Bus queue **messagequeue** in the Azure portal.

1.  Review the **Essentials** pane and note that the queue contains 3 active messages.
...
1.  On the **Service Bus Explorer (preview)** blade, select the **Peek** tab header and, on the **Peek** tab, select **Peek** button.

1.  Verify that the queue contains 3 messages. 
...
1.  Enter the following code to create a static async **MessageHandler** task which will display body of messages in the queue as they are being processed and deletes them once the processing completes:
...
1.  Enter the following code to create a static async **ErrorHandler** task which will handle any exceptions encountered during message processing:
...
1.  In the **Main** method, add the following code to initialize *client* of type **ServiceBusClient** that will provide connectivity to the Service Bus namespace and **processor** that will be responsible for processing of messages:
    ...
    > **Note**: As mentioned earlier, the Service Bus client is safe to cache and use as a singleton for the lifetime of the application. This is considered one of the best practices when publishing and reading messages on regular basis.
...
1.  Verify that the console message displayed at the terminal prompt states that each of the 3 messages in the queue has been received.
...
1.  On the **Service Bus Explorer (preview)** blade, select **Refresh** and note that the number of active messages in the queue changed to **0**.
...
1.  From the terminal prompt, run the following command to delete the **AsyncProcessor** resource group:
