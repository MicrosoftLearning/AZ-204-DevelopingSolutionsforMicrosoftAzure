...
1.  On the **Create namespace** blade, on the **Basics** tab, perform the following actions, and select **Review + create**:
    ...
    | **Resource group** section | Select existing resource group **AsyncProcessor-[Deploymentid]**|
    | **Namespace name** text box | Enter **sbnamespace[Deploymentid]**|
...
1.  In the **Open Folder** window, browse to **C:\AllFiles\AZ-204-DevelopingSolutionsforMicrosoftAzure-master\Allfiles\Labs\10\Solution\\MessagePublisher**, and then select **Select Folder**.
...
1.  In the **Open Folder** window, browse to **C:\AllFiles\AZ-204-DevelopingSolutionsforMicrosoftAzure-master\Allfiles\Labs\10\\Solution\MessageReader**, and then select **Select Folder**.
...
1.  In the **Main** method, Observe the following code to initialize *client* of type **ServiceBusClient** that will provide connectivity to the Service Bus namespace and **processor** that will be responsible for processing of messages:
...
1.  In the **Main** method, Observe the following lines of code to create a try block, which first implements a message and error processing handler, initiates message processing, and stops processing following a user input:
