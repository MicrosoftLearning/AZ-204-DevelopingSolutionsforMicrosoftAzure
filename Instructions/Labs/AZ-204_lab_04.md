...
1.  Note the **using** statement that relies on a **StreamReader** object to read JSON items from a text file. <!-- ID/SME: Please note, I broke the original one sentence into two. Please confirm I didn't mess up the technical veracity. Thanks! -->It deserializes them into objects of the **Model** class defined further in the **Program.cs** file.
