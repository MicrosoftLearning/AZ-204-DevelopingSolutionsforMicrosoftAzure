...
1.  In the Azure portal, use the **Search resources, services, and docs** text box at the top of the page to search for **Storage accounts** and, in the list of results, select **Storage accounts**.
...
1.  From the **Review + Create** tab of the **Create a storage account** blade, review the options that you selected during the previous steps.
...
In this exercise, you created the Azure resources that you'll need for the polyglot data solution you will implement in this lab.

### Exercise 2: Review and upload data

#### Task 1: Upload images to Azure Blob storage

1.  In the Azure portal's navigation pane, navigate back to the **Storage accounts** blade and select the **polystor**_[yourname]_ storage account that you created in the previous exercise of this lab.
...
1.  On line 15, set the value of **AutohrizationKey** by replacing the empty string with the **PRIMARY KEY** property of the Cosmos DB account that you recorded earlier in this lab (ensure that the value is enclosed in double quotes).
...
1.  Note the invocation of the **DefineContainer** method of the **Database** class. This will create a container that will host the JSON items, if one does not already exist.
    ...
    > **Note**: The **DefineContainer** method includes the option that modify the default indexing policy (which automatically indexes all attributes) to minimize cost.
...
1.  Save an close the **Program.cs** file.
...
1.  Observe the results of the execution printed in the terminal. The run should complete successfully, displaying the message about 119 items inserted into the target Cosmos DB collection.
...
1.  In the Azure portal, navigate back to the **Azure Cosmos DB** blade and, in the listing of Cosmos DB accounts, select the **polycosmos**_[yourname]_ Azure Cosmos DB account that you created earlier in this lab.
...
1.  From the **Touring-1000** product detail page review the listing of options.
