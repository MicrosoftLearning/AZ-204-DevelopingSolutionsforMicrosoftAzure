...
1.  From the **Azure Cosmos DB** blade, select **+ Create**.

1.  From the **Select API option** blade, select **Core (SQL) - Recommended**.

1.  On the **Basics** tab of the **Create Azure Cosmos DB Account - Core (SQL)** blade, perform the following actions:
    
    1.  Retain the default value for the **Subscription** list. <!-- Marcin, please check if this edit is okay.-->
    
    1.  In the **Resource group** section, select **Create new**, and then in the **Name** text box, type **PolyglotData**, and select **OK**.
    
    1.  In the **AccountName** text box, enter **polycosmos**_[yourname]_.
    
    1.  In the **Location** drop-down list, select an Azure region that is closest to the location of your lab computer and where you can create a Cosmos DB account. <!--Marcin, I reordered this sentence a bit to put teh location bit first - is that okay? I wanted to avoid using both which and where in the same sentence.-->
    
    1.  In the **Capacity mode** section, select **Serverless**, and then select **Review + Create**. <!--Marcin, I combined two steps into one-->
...
1.  From the **Azure Cosmos DB account** blade, find the **Settings** section, and then select the **Keys** link.
...
1.  In the Azure portal, use the **Search resources, services, and docs** text box <!--Marcin, not sure if it is okay to just ask them to search for "Storage accounts in the Azure portal, without specifiying the text box name?  We already told them how to do that in prev exercise. I have not changed anything but just checking with verbosity in mind.-->to search for **Storage accounts** and, in the list of results, select **Storage accounts**.

1.  From the **Storage accounts** blade, select **+ Create**.

1.  From the **Basics** tab of the **Create a storage account** blade, perform the following actions:
    
    1.  Retain the default value for the **Subscription** list.
    ...
    1.  In the **Region** drop-down list, select the same region where you created the Cosmos DB account earlier in this exercise.
...
1.  From the **polystor**_[yourname]_ storage account blade, select the **Containers** link in the **Data storage** section.
...
1.  Find the **Settings** section in the **Container** blade, and then select the **Properties** link. <!--Marcin, please check edits.-->
...
1.  From the terminal prompt, run the following command to add the Azure Cosmos DB .NET client library to the currently opened project: <!--Marcin, can we avoid repeating "from the terminal prompt" again--> 
...
1.  Note the using statement that relies on a **StreamReader** <!-Marcin, I made this bold. Should "using" also be bold or in code format?--> object to read JSON items from a text file and deserializes them into objects of the **Model** class defined further in the **Program.cs** file.
