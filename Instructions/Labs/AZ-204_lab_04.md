...
#### Task 1: Upload images to Azure Blob Storage <!-- ID/SME: Per Microsoft AI & Cloud Style Guide, it's "Azure Blob Storage" (note capitalization). Please make sure all instances are fixed globally. -->
...
1.  Note the invocation of the **DefineContainer** method of the **Database** class. This will create a container that will host the JSON items, if one doesn't already exist.
    ...
    > **Note**: The **DefineContainer** method includes a cost-minimizing option whereby you cab modify the default indexing policy (which automatically indexes all attributes).
