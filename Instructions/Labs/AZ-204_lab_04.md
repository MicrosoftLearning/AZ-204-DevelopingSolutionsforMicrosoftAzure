...
1.  In the Azure portal, Sign in with the credentials provided in the evnironment tab

2.  use the **Search resources, services, and docs** text box to search for **Azure Cosmos DB** and then in the list of results, select **Azure Cosmos DB**.

3.  On the **Azure Cosmos DB** blade, select **+ Create**.

4.  On the **Select API option** blade, select **Core (SQL) - Recommended**.

5.  On the **Basics** tab of the **Create Azure Cosmos DB Account - Core (SQL)** blade, perform the following actions, and then select **Review + Create**:

    | Setting                     | Action                                                       |
    | --------------------------- | ------------------------------------------------------------ |
    | **Subscription** list       | Retain defaults.                                             |
    | **Resource group** section  | Select **Existing rg** PolyglotDataDID                       |
    | **AccountName** text box    | Enter **polycosmosDID**                                      |
    ...
    >Note : Deployment ID(DID) can be found in the environment tab
...
1.  On the **Basics** tab of the **Create a storage account** blade, perform the following actions, and then select **Review + Create**:
    ...
    | **Storage account name** text box | Enter **polystorDID**.                                 |
    ...
    | **Redundancy** drop-down list | Select **Locally-redundant storage (LRS)**. |
     ...
     >Note : DeploymentID (DID) can be found under environment tab
...
1.  In the **Upload blob** pop-up, perform the following actions:
    ...
    b.  In the **File Explorer** window, browse to **Allfiles C:\\AZ-204-DevelopingSolutionsforMicrosoftAzure\\Allfiles\\Labs\\04\\Starter\\AdventureWorks**, select all 42 individual **.jpg** image files, and then select **Open**.
...
1.  From the **File** menu, select **Open File**, browse to **Allfiles C:\\AZ-204DevelopingSolutionsforMicrosoftAzure\\Allfiles\\Labs\\04\\AdventureWorks\\AdventureWorks.Upload**, select **models.json**, and then select **Open**.
...
1.  In the **File Explorer** window that opens, browse to **Allfiles C:\\Allfiles\\Labs\\04\\Starter\\AdventureWorks**, and then select **Select Folder**.
...
1.  On line 19, set the value of **JsonFilePath** by replacing the empty string with **"C:\\AZ-204-DevelopingSolutionsforMicrosoftAzure\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Models\\Models.json"**.

>Note : Verify the JsonPath accordingly
