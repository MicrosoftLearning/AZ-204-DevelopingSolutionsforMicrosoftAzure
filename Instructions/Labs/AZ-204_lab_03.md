...
1.  1.  In the Azure portal, use the **Search resources, services, and docs** text box to search for **Storage Accounts**, and then in the list of results, select **Storage Accounts**.
...
1.  On the **Storage accounts** blade, select the **mediastor**_[yourname]_ storage account instance.
    ...
    > **Note**: If the **mediastor**_[yourname]_ storage account instance doesn't appear, select **Refresh**.
...
1. On the **StorageMedia** blade, select the **mediastor**_[yourname]_ storage account that you created earlier in this lab.
...
In this exercise, you created a couple of placeholder containers in the storage account, and then populated one of the containers with a blob.
...
1.  Update the **storageAccountKey** string constant by setting its value to the **Key** of the storage account <!--Conan, I changed to lower case here for storage. Hope that's ok.--> that you recorded earlier in this lab.
...
1.  In the **Main** method, add the following line of code to create a new instance of the **BlobServiceClient** class by using the **blobServiceEndpoint** constant and the *accountCredentials* variable as constructor parameters: <!--Conan, do we need to repeat the "In the Main method for each of these steps? Is it possible to just state at the beginning of the task that all of these lines of code are being added to the Main method? It seems repetitive-->
...
<!--Conan, do you mean the code has actually been removed from the final program or that we are not displaying or repeating it here for brevity?-->
...
1.  In the **Cloud Shell** pane, run the following command to delete the **StorageMedia** resource group:
