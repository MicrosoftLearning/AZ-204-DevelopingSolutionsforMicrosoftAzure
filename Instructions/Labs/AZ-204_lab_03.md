...
1. Click on the Azure Portal icon on the VM desktop (<https://portal.azure.com>) and login with the Azure credentials from the Lab Environment output page.
...
1.  On the **Create a storage account** blade, on the **Basics** tab, perform the following actions, and then select **Review + create**:
   ...
   | **Resource group** section        | Select **Create new**, enter **StorageMedia-[Deploymentid]**, and then select **OK**. |
   | **Storage account name** text box | Enter **mediastor**_[Deploymentid]_.                             |
...
1.  On the **Storage accounts** blade, select the **mediastor**_[Deploymentid]_ storage account instance.

    > **Note**: If the **mediastor**_[Deploymentid]_ storage account instance doesn't display, select **Refresh**.
...
1. On the **StorageMedia** blade, select the **mediastor**_[Deploymentid]_ storage account that you created previously in this lab.
...
1.	In the **Upload blob** window, perform the following actions, and then select **Upload**:
   ...
   | **File Explorer** window        | Browse to **Allfiles (C):\\Allfiles\\Labs\\03\\Starter\\Images**, select the **graph.jpg** file, and then select **Open**. |
...
1.  On the **File** menu, select **Open Folder**, browse to **Allfiles (C):\\Allfiles\\Labs\\03\\Starter\\BlobManager**, and then select **Select Folder**.
...
1.  At the open command prompt, run the following command to run the .NET web application:
    ...
    > **Note**: If there are any build errors, review the **Program.cs** file in the **Allfiles (C):\\Allfiles\\Labs\\03\\Solution\\BlobManager** folder.
...
1.	In the **Upload blob** window, perform the following actions, and then select **Upload**:
    ...
    | --------------------------------- | ------------------------------------------------------------ |
    | **Files** section   | Select the **Folder** icon.                                    |
    | **File Explorer** window        |  **Allfiles (C):\\Allfiles\\Labs\\03\\Starter\\Images**, select the **graph.svg** file, and then select **Open**. |
    | **Overwrite if files already exist** check box | Ensure that the check box is selected.                        |
