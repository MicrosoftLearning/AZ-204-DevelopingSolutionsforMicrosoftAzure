...
1.  Run the following command command, ensuring you see the name of your registry as output. If you see no output other than '[]', wait a minute and try running the command again:
...
1.  On the **Create Container Instance** blade, on the **Basics** tab, perform the following actions, and then select **Review + create**:
    ...
    > **Note**: After selecting Azure container registry, verify Registry, Image and Image tag are automatically populated with the one which we created in previous task.
