...
1.  From the **Application Insights Deployment** blade, select **Go to resource**

1.  From the **Application Insights** blade, in the **Configure** category, select the **Properties** link.

1.  In the **Properties** section, find the value of the **Instrumentation Key** text box. This key is used by client applications to connect to Application Insights. Record the value of the **Instrumentation Key**, you'll use this value later in this lab.
...
1.  From the **Deployment** tab, perform the following actions:
...
1.  From the **Web App Deployment** blade, select **Go to resource**.
...
1.  In the **Visual Studio Code** window, right-click the Explorer pane or activate the shortcut menu, and then select **Open in Integrated Terminal**.
...
1.  At the command prompt, enter the following command, and then select Enter to create a certificate. Click **YES** on the prompted option.
