---
lab:
    az204Title: 'Lab 08: Creating a multi-tier solution by using Azure services'
    az020Title: 'Lab 08: Creating a multi-tier solution by using Azure services'
    az204Module: 'Module 08: Implement API Management'
    az020Module: 'Module 08: Implement API Management'
---

# Lab 08: Creating a multi-tier solution by using Azure services
...
1.  Within the web application, perform the following actions:
       ...
       The following screenshot displays the response section of the web application:
...
1.  Within the web application, perform the following actions:
    ...
    b.  Select **GET /basic-auth/{user}/{passwd}**.

    c.  Select **Try it out**.

    d.  In the **user** text box, enter **user**.

    e.  In the **passwd** text box, enter **passwd**.

    f.  Select **Execute**.

    g.  When prompted, in the **Username** text box, enter **user**, in the **Password** text box, enter **passwd**, and then select **Sign in**.

    h.  Review the value of the **Response body** and **Response headers** text boxes.

    i.  Review the value of the **Request URL** text box.
    
    The following screenshot displays the authentication response section of the web application:
...
1.  In the **Set Headers** section, perform the following actions, and then select **Save**:
    ...
    The following screenshot displays the configured settings of the **Set Headers** section.
...
1.  Review the results of the API request.
    ...
    > **Note**: Verify that the many headers sent as part of your request are echoed in the response. They should include the new **Source** header that you created as part of this task.
...
1.  In the **Rewrite URL** section, perform the following actions:
    ...
    a.  In the **Backend** text box, enter **/basic-auth/user/passwd**.
...
1.  Review the results of the API request.
    ...
    > **Note**: Verify that the request returned the **HTTP/1.1 200 OK** response.
       ...
       The following screenshot displays the response to the basic authentication request:
...
1.  In the Azure portal, select the **Cloud Shell** icon to open a new PowerShell session. If Cloud Shell defaults to a PowerShell session, select **PowerShell** and then, in the drop-down menu, select **Bash**.
...
1.  From the  Cloud Shell pane, run the following command to delete the **ApiService** resource group:
    ...
    ```
     ...
     > **Note**: The command executes asynchronously (as determined by the --no-wait parameter <!-- ID/SME: parameters should be italic. -->), so while you'll be able to run another Azure CLI command immediately afterwards within the same Bash session, it'll take a few minutes before the resource groups are actually removed.
