...
1.  In the **Add operation** section, perform the following actions, and then select **Save**:
    ...
    | **URL** list | Select **GET** |
    | **URL** text box | Enter **/json** |


1.  Back on the **Design** tab, in the list of operations, select **Generate Payload**, and then select the **Test** tab.

1.  In the **Generate Payload** section, select **Send**.

1.  Review the results of the API request and then select the **Generate definition** link.

1.  In the **Create new definition** pane, in the **Definition name** text box, type **httpbinJSON** and then select the **Create definition** button.

       ![A JSON definition based on the sample payload](./media/l08_create_json_definition.png)

1.  Back on the **Design** tab, select the **Definitions** tab and, in the list of definitions, select **httpbinJSON**.

1.  On the **Definition** pane, in the **Definition body** section, ensure that the **Schema** tab is selected, select the schema definition, and copy it into Clipboard.

1.  On the Azure portal page displaying the list of APIs, in the vertical menu bar, in the **APIs** section, select **Schemas**.

1.  On the **Schemas** page, select **+ Add**.

1.  On the **Create schema** page, in the **Name** text box, type **httpbinJSON**, ensure that the **Create new** entry is selected in the **Create method** drop-down list, paste the content of Clipboard into the **Content** pane, and select **Save**.

    > **Note**: The schema should have the following format:
...
1.  On the Azure portal page displaying the list of APIs, in the vertical menu bar, in the **APIs** section, select **APIs**.

1.  In the list of APIs, select **HTTPBin API**. 

1.  On the **Design** tab, in the list of operations, select **Generate Payload**.

1.  On the **Outbound processing** tile, select **+ Add policy**.

1.  In the **Add outbound policy** section, select the **Validate content** tile.

1.  In the **Outbound processing** section, select the **Full** link, perform the following actions, and then select **Save**:
    
    | Setting | Action |
    | ---- | ----- |
    | **Unspecified content type action** drop-down list | Select **Prevent** |
    ...
    | **Content type validation** **Type** text box | Enter **application/json** |
    | **Content type validation** **SchemaID** text box | Enter **httpbinJSON** |
    | **Content type validation** **Validate as** drop-down list | Select **JSON** |
    | **Content type validation** **Action** drop-down list | Select **Prevent** |
