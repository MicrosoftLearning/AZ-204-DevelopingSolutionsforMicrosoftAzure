...
1.  In the **Backend** section, perform the following actions, and then select **Save**:
    ...
    | **Service URL** text box | Append the value **/headers** to its current value. **Note**: For example, if the current value is **http://httpapi[DeploymentID].azurewebsites.net**, the new value will be **http://httpapi[DeploymentID].azurewebsites.net/headers** |
