...
1.  On the **Create Web App** blade, on the **Basics** tab, perform the following actions:
    ...
    | **Resource group** section      | Select Existing rg, enter **ApiServicedeDeployment ID**,  and then select **OK**. |
    | **Name** text box               | Enter **httpapiDeployment ID** .                        |
...
1.  Switch back to the browser window that displays the **httpapiDeployment ID** web app.
...
1.  On the **Create API Management** blade, perform the following actions, and then select **Review + create**:
    ...
    | --------------------------------- | ------------------------------------------------------------ |
    | **Subscription** drop-down list    | Retain the default value.                                    |
    | **Resource group** section        | Select the **ApiServiceDeployment ID** group that you created  earlier in the lab. |
    | **Resource name** text box        | Enter **proapiDeployment ID** .                               |
...
1.  In the **Backend** section, perform the following actions, and then select **Save**:
    ...
    | **Service URL** text box | Append the value **/headers** to its current value. **Note**: For example, if the current value is `http://httpapiDeployment ID.azurewebsites.net`, the new value will be `http://httpapiDeployment ID.azurewebsites.net/headers`.|
