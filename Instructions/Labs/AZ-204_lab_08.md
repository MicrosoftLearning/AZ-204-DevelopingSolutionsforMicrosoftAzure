...
1.  Within the web application, perform the following actions:
    ...
    d.  In the user text box, enter **user**. <!--Marcin, doesn't the text box name appear in the UI? if it appears, we should bold "user" and "passwd" in the next step.-->

    e.  In the passwd text box, enter **passwd**.

    f.  Select **Execute**.

    g.  When prompted, in the **Username** text box, enter **user**, in the **Password** text box, enter **passwd**, and select **Sign in**. <!--Marcin, are they entering the same credentials twice? Just checking-->
