...
1.  In the **Backend** section, perform the following actions, and then select **Save**:
    ...
    | **Service URL** text box | Append the value **/headers** to its current value. **Note**: For example, if the current value is `http://httpapi<inject key="DeploymentID" enableCopy="false"/>.azurewebsites.net`, the new value will be `http://httpapi[deploymentID].azurewebsites.net/headers`|
...
1.  Back on the **Design** tab, in the list of operations, select **Get Legacy Data**.
    ...
    > **Note**: Ensure to check the Override checkbox in the backend panel and proceed with the next step.
