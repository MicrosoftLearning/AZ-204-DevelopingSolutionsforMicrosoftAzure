...
1.  Within the web application, perform the following actions:
    ...
    e.  Select **Execute**.
        ...
        ![image](./media/Lab-08_Ex1_Task3_1.png)
...
1.  Within the web application, perform the following actions:
    ...
    e.  Select **Execute**.
        ...
        ![image](./media/Lab-08_Ex1_Task3_2.png)
...
1.  In the **properties** section, record the value of the **default domain URL** link under Domain. You'll use this value later in the lab to send requests to the corresponding API.
...
1.  In the **Create a HTTP API** window, perform the following actions in the **Basic** tab and select **Create**:
    ...
    | **Web service URL** text box      | Enter the URL for the web app that you copied earlier in this lab **Note**: Make sure that the URL starts with the http:// prefix.|     | **API URL suffix** text box   | Leave it empty  |
...
1.  In the **Backend** section, perform the following actions, and then select **Save**:
    ...
    | **Service URL** text box | Append the value **/headers** to its current value. **Note**: For example, if the current value is **http://httpapi<inject key="DeploymentID" enableCopy="false"/>.azurewebsites.net**, the new value will be **http://httpapi<inject key="DeploymentID" enableCopy="false"/>.azurewebsites.net/headers** |
