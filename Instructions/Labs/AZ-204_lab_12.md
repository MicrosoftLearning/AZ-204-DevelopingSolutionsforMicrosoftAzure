...
1. In the Azure portal, use the **Search resources, services, and docs** text box to search for **Storage account**, and then in the list of results, select **Storage account**.

1. On the **Storage account** blade, select **+ Create**.
...
1. On the **Create a storage account** blade, on the **Basics** tab, perform the following actions, and then select **Review + create**.
   ...
   | **Resource group** section        | Select **Create new**, enter **MarketingContent**_[DeploymentId]_, and then select **OK**. |
   | **Storage account name** text box | Enter **contenthost**_[DeploymentId]_.                           |
...
1. On the **Create Web App** blade, on the **Basics** tab, perform the following actions, and then select **Next: Docker**:
   ...
   | --------------------------------- | ------------------------------------------------------------ |
   | **Subscription** drop-down list   | Retain the default value.                                    |
   | **Resource group** drop-down list | Select **MarketingContent**_[DeploymentId]_ in the list.                     |
   | **Name** text box                 | Enter **landingpage**_[DeploymentId]_.                           |
...
1. On the **Resource groups** blade, select the **MarketingContent**_[DeploymentId]_ resource group that you created previously in this lab.

1. On the **MarketingContent**_[DeploymentId]_ blade, select the **landingpage**_[DeploymentId]_ web app that you created previously in this lab.
...
1. On the **MarketingContent**_[DeploymentId]_ blade, select the **contenthost**_[DeploymentId]_ storage account that you created previously in this lab.
...
1. On the **MarketingContent**_[DeploymentId]_ blade, select the **contentdeliverynetwork** CDN profile that you created previously in this lab.
...
1. In the **Add an endpoint** pop-up dialog box, perform the following actions, and then select **Add**:
   ...
   | Setting                                   | Action                                                       |
   | ----------------------------------------- | ------------------------------------------------------------ |
   | **Name** text box                         | Enter **cdnmedia**_[DeploymentId]_.                              |
   | **Origin type** drop-down list            | Select **Storage**.                                          |
   | **Origin hostname** drop-down list        | Select the **contenthost*[DeploymentId]*.blob.core.windows.net** option for the Storage account that you created previously in this lab. |
...
1. In the **Add an endpoint** pop-up dialog box, perform the following actions, and then select **Add**:
   ...
   | Setting                                   | Action                                                       |
   | ----------------------------------------- | ------------------------------------------------------------ |
   | **Name** text box                         | Enter **cdnvideo**_[DeploymentId]_.                              |
   | **Origin type** drop-down list            | Select **Storage**.                                          |
   | **Origin hostname** drop-down list        | Select the **contenthost*[DeploymentId]*.blob.core.windows.net** option for the Storage account that you created previously in this lab. |
...
1. In the **Add an endpoint** pop-up dialog box, perform the following actions, and then select **Add**:
   ...
   | Setting                                   | Action                                                       |
   | ----------------------------------------- | ------------------------------------------------------------ |
   | **Name** text box                         | Enter **cdnweb**_[DeploymentId]_.                                |
   | **Origin type** drop-down list            | Select **Web App**.                                          |
   | **Origin hostname** drop-down list        | Select the **landingpage*[DeploymentId]*.azurewebsites.net** option for the web app that you created previously in this lab. |
...
1. On the **MarketingContent** blade, select the **landingpage**_[DeploymentId]_ web app that you created previously in this lab.
...
1. In the **Add/Edit application setting** pop-up window, configure the following settings, and then select **OK**.
   ...
   | **Value** text box                    | Enter the **URI** value of the **media** container in the **contenthost**_[DeploymentId]_ storage account that you recorded previously in this lab. |
...
1. In the **Add/Edit application setting** pop-up window,configure the following settings, and then select **OK**.
   ...
   | **Value** text box                    | Enter the **URI** value of the **video** container in the **contenthost**_[DeploymentId]_ storage account that you recorded previously in this lab. |
   | **Deployment slot setting** check box | Don't select.                                               |
...
1. On the **CDN profile** blade, select the **cdnmedia**_[DeploymentId]_ endpoint.
...
1. On the **CDN profile** blade, select the **cdnvideo**_[DeploymentId]_ endpoint.
...
1. Construct a URL for the **campus.jpg** resource by combining the **Endpoint hostname** URL from the **cdnmedia**_[DeploymentId]_ endpoint that you copied previously in the lab with a relative path of **/campus.jpg**.
...
1. Construct a URL for the **conference.jpg** resource by combining the **Endpoint hostname** URL from the **cdnmedia**_[DeploymentId]_ endpoint that you copied previously in the lab with a relative path of **/conference.jpg**.
...
1. Construct a URL for the **poster.jpg** resource by combining the **Endpoint hostname** URL from the **cdnmedia**_[DeploymentId]_ endpoint that you copied previously in the lab with a relative path of **/poster.jpg**.
...
1. Construct a URL for the **welcome.mp4** resource by combining the **Endpoint hostname** URL from the **cdnvideo**_[DeploymentId]_ endpoint that you copied previously in the lab with a relative path of **/welcome.mp4**.
...
1. In the **Add/Edit application setting** pop-up dialog box, update the **Value** text box by entering the **Endpoint hostname** URL from the **cdnmedia**_[DeploymentId]_ endpoint that you copied previously in the lab, and then select **OK**.

1. Select the existing **CDNVideoEndpoint** application setting.

1. In the **Add/Edit application setting** pop-up dialog box, update the **Value** text box by entering the **Endpoint hostname** URL from the **cdnvideo**_[DeploymentId]_ endpoint that you copied previously in the lab, and then select **OK**.
...
1. On the **CDN profile** blade, select the **cdnweb**_[DeploymentId]_ endpoint.
...
1. In the new browser window, refer to the **Endpoint hostname** URL for the **cdnweb**_[DeploymentId]_ endpoint.
