...
1.  In the **Register an application** section, perform the following actions:
    ...
    1.  In the **Supported account types** list, select the **Accounts in this organizational directory only (Default Directory only - Single tenant)** option.
...
1.  In the **Authentication** section, perform the following actions:
    ...
    1.  On the **Configure Web** blade, perform the following actions:

        1.  In the **Redirect URIs** text box, type **https://localhost:44321/**

        1.  In the **Front-channel logout URL** text box, type **https://localhost:44321/signout-oidc**

    1.  Select **Configure**.

    1.  Back in the **Platform configurations** section, select **Add URI** and type **https://localhost:44321/signin-oidc**.

    1.  In the **Implicit grant and hybrid flows** section, select **ID tokens (used for implicity and hybrid flows)**.
...
1.  Navigate to the **Properties** folder, open the **launchSettings.json** file, and then apply the following changes:
    ...
    1.  Set the value of the **sslPort** property in the **iisSettings** section to **44321**.

    1.  Set the value of the **applicationUrl** property in the **OIDCClient** section to **https://localhost:44321**.
...
1.  Open the file **appsettings.json** and review the content of the **AzureAd** object, including the following elements:
    ...
    1.  `Instance` set to `https://login.microsoftonline.com/`
    1.  `Domain` set to the primary DNS domain of the Azure AD tenant associated with your Azure subscription.
    1.  `TenantId`set to the GUID of the Azure AD tenant
    1.  `ClientId`set to the Application (client) ID of the application you registered in the Azure AD tenant
    1.  `CallbackPath` set to `/signin-oidc`
...
1.  Run the following command to generate a self-signed certificate and automatically configure the local computer to trust it: <!-- Marcin, should this be "Run the following command to generate a self-signed certificate and configure the local computer to automatically trust it."?-->
...
1.  In the open browser window, when prompted, authenticate by using the user principal name of the **aadlabuser1** Azure AD account you created earlier in this lab with **Pa55w.rd1234** as its password. <!--Marcin, can we simplify this step as follows: In the open browser window, when prompted, authenticate by using the following credentials of the Azure AD account you created earlier in this lab: -->
