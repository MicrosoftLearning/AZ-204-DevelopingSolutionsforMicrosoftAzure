...
1.  Run the following commands to create Azure AD users that you'll use to test Azure AD authentication:
    ...
    ```powershell
       ...
       New-AzureADUser -AccountEnabled $true -DisplayName 'aad_lab_user1' -PasswordProfile $passwordProfile -MailNickName 'aad_lab_user1' -UserPrincipalName
      ```
...
1.  From the command prompt, run the following commands to create and set the working directory to **Allfiles (C):\\Allfiles\\Labs\\06\\Solutiom\\OIDCClientST**:
    ...
    ```powershell
     C:
     md "C:\AZ-204-DevelopingSolutionsforMicrosoftAzure\Allfiles\Labs\06\Solution" 
     cd C:\AZ-204-DevelopingSolutionsforMicrosoftAzure\Allfiles\Labs\06\Solution
    ```
...
1.  In the **File Explorer** window, browse to **Allfiles (C):\\Allfiles\\Labs\\06\\Solution\\OIDCClient**, and then select **Select Folder**.
...
1.  Run the following commands to create Azure AD users that you'll use to test Azure AD authentication:
    ...
    ```powershell
       ...
       $passwordProfile.Password = 'Pa55w.rd1234'
       $passwordProfile.ForceChangePasswordNextLogin = $false
       New-AzureADUser -AccountEnabled $true -DisplayName 'aad_lab_user2' -PasswordProfile $passwordProfile -MailNickName 'aad_lab_user2' -UserPrincipalName
    ```
