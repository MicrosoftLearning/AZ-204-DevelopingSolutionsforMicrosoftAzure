...
1.  Run the following to create Azure AD users that you will use to test Azure AD authentication:
...
1.  Run the following to identify the user principal name of the newly created Azure AD user:
...
1.  Navigate to the **Properties** folder and open the **launchSettings.json** file and apply the following changes:
...
1.  Add the following using directives directly below the entry removed in the previous step:
...
1.  Replace the newly removed lines with the following ones:
...
1.  Replace the newly removed line with the following ones:
...
1.  When prompted whether to install the autogenerated certificate, select **Yes**.
...
In this exercise, you implemented a single-tenant web app and tested in in a single-tenant Azure environment.
...
1.  Switch to the Azure portal and select the **Directories + subscriptions** toolbar icon. 
...
1.  Replace the line containing `TenantId` element with the following one:
