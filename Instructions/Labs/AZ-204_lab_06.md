...
1. To get the Azure credentials select the **Environment Details** tab from the lab environment details page.
    ...
    > **Note**: If there is any popup for Action required, then select **Ask me later** and for Stay signed in Popup, select **No** and **May be later** for Azure tour window.
...
1.  In the **Register an application** section, perform the following actions, and then select **Register**:
    ...
    | **Supported account types** list | Select **Accounts in this organizational directory only (Azure HOL XXXX only - Single tenant)** |

   
    The following screenshot displays the configured settings in the **Register an application** section.
          
     ![Screenshot displaying the options configured to register an application.](./media/AZ-204-M6.png)
...
1.  In the **Authentication** section, perform the following actions, and select **Configure**:
    ...
    | **Implicit grant and hybrid flows** section | select **ID tokens (used for implicit and hybrid  flows)** checkbox |
...
1.  In the Azure portal, select the **Cloud Shell** icon ![Cloud Shell icon](./media/az204_lab_CloudShell.png).
    ...
    > **Note**:  If this is the first time you're starting Cloud Shell, when prompted to select either Bash or PowerShell, select **Powershell**. When the You have no storage mounted message appears, select Show advanced settings and then select Use existing and choose existing resource group. Then select Create new against Storage account and enter **cloudstor<inject key="DeploymentID" enableCopy="false"/>** and also for File Share,enter **blob<inject key="DeploymentID" enableCopy="false"/>** and then click on Create storage, and wait for the Azure Cloud Shell to initialize. and then select **Create storage**.
...
1. If you have been prompted **Do you trust the authors of all files in this folder?**, select **Yes, I trust the authors**.
